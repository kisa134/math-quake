...
// ── cs «ПЕСЧАНКА» — ПРОСТАЯ КАРТА В СТИЛЕ КОНТРЫ ───────────────────────────
/**
 * Читается с первого захода, как de_dust: два спавна друг напротив друга, МИД
 * посередине, две точки — A справа, B слева, к каждой два входа. Никаких
 * лабиринтов и вертикальных этажей: один уровень, ящики для укрытий, короткие
 * связки. Вся геометрия — коробки, потолок над всем.
 */
function buildCS(): QMapData {
  const H = 40;                                   // высота стен
  const pieces: QPiece[] = [slab(0, 0, 0, 340, 340, 4)];
  shell(pieces, 0, 0, 340, 340, H);               // внешняя коробка + потолок

  // --- внутренние стены: две «косынки» делят карту на мид + два фланга ---
  // блок между мидом и точкой A (справа) с проходом в середине
  const block = (x: number, z: number, w: number, d: number) =>
    pieces.push(wall(x, 0, z, w, H, d, { metal: false }));

  // мид-коридор шириной 34 по центру: две длинные стены с дырами-выходами
  block(-24, -66, 4, 90);   block(-24, 66, 4, 90);    // левая стена мида (щель у центра)
  block(24, -66, 4, 90);    block(24, 66, 4, 90);     // правая стена мида

  // фланговые стены точек
  block(-104, -34, 4, 96);  block(104, -34, 4, 96);
  block(-104, 96, 4, 70);   block(104, 96, 4, 70);

  // поперечины у спавнов (короткие связки CT→A/B)
  block(-70, -120, 64, 4);  block(70, -120, 64, 4);

  // --- ТОЧКА A (справа) и ТОЧКА B (слева): платформа + ящики ---
  const site = (sx: number) => {
    pieces.push(slab(sx, 6, -40, 54, 54, 6, { glow: true }));      // подиум точки
    pieces.push(wall(sx - 22, 6, -18, 12, 12, 12, { metal: true }));
    pieces.push(wall(sx + 20, 6, -60, 12, 12, 12, { metal: true }));
    pieces.push(wall(sx + 2, 6, -40, 14, 20, 14, { metal: true })); // высокий ящик
  };
  site(120); site(-120);

  // --- укрытия на миде: ящики в шахматном порядке (перестрелка «через мид») ---
  for (const [bx, bz] of [[-10, 20], [12, -14], [-6, -46], [10, 52], [-14, -78]] as [number, number][]) {
    pieces.push(wall(bx, 0, bz, 12, 11, 12, { metal: true }));
  }

  // --- спавны: «Т» внизу (+z), «КТ» вверху (−z) ---
  pieces.push(slab(0, 1.2, 148, 120, 44, 1.2, { glow: true }));    // площадка Т
  pieces.push(slab(0, 1.2, -148, 120, 44, 1.2, { glow: true }));   // площадка КТ

  const pads: QPad[] = [
    { p: [-150, 2, 148], force: lift(16) },   // короткий заброс на фланг B
    { p: [150, 2, 148], force: lift(16) },    // и на фланг A
  ];

  return {
    pieces, pads,
    spawns: [
      [0, 4, 150], [-40, 4, 150], [40, 4, 150],
      [0, 4, -150], [-40, 4, -150], [40, 4, -150],
      [120, 10, -40], [-120, 10, -40],
    ],
    playerSpawn: [0, 5, 148],
    orbs: [
      { p: [120, 10, -40], kind: 'buff' },     // на точке A
      { p: [-120, 10, -40], kind: 'buff' },    // на точке B
      { p: [0, 4, 0], kind: 'cash' },          // центр мида — «за деньгами в мид»
    ],
  };
}

export const QUAKE_MAPS: Record<Exclude<MapId, 'donut' | 'tower'>, QMapData> = {
  q1: buildQ1(), q2: buildQ2(), q3: buildQ3(), q4: buildQ4(), q5: buildQ5(), cs: buildCS(),
};
